...
def ubuntu_os_config(new_server_entry:str):
    ...
    setup_script = "/data/automation/os_config/ubuntu_os_config/setup_keys.yml"
    config_script = "ansible-playbook -i server.txt main.yml"
    ansible_cmd = ["ansible-playbook", "-i", server_file, "main.yml"]
    ansible_key = ["ansible-playbook", "-i", server_file, "setup_keys.yml", "--vault-password-file", "vault.pass"]
    ...
    try:
        ...
        print("Step 2: Running setup_keys")
        #result = subprocess.run(["bash", setup_script], text=True, capture_output=True)
        result = subprocess.run(ansible_key, text=True, capture_output=True)
        if result.returncode != 0:
            logging.error("setup_keys failed: %s", (result.stderr or result.stdout))
            return (result.stderr or result.stdout) or "setup_keys failed"
        ...
        output_file = "/data/automation/os_config/ubuntu_os_config/ansible_output/"+new_hostname+"/status.txt"
...
def process_tickets_cycle() -> None:
    ...
    for tic in tickets_open:
        try:
           ...
           # tic_data[ticket_num] = [os_type, host_name]
            tic_data[ticket_num] = [os_type, host_name.strip()]
    ...
    for val, (os_info, host) in tic_data.items():
        ...
        if 'Red Hat' in os_info:
            #if val == "TASK1276383":
            worknotes = "RPA Tool Working in progress"
            task_update(val, worknotes, stat_wip, assign_to, worknotes)
